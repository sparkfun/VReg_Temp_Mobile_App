...
// This code comes from the Xamarin.FormsBook.Toolkit, but was modified slightly

namespace VoltageRegulatorTemperature.Behaviors
{
	public class UnitsRadioBehavior : Behavior<View>
	{
		TapGestureRecognizer tapRecognizer;
		static List<UnitsRadioBehavior> defaultGroup = new List<UnitsRadioBehavior>();
		static Dictionary<string, List<UnitsRadioBehavior>> radioGroups =
									new Dictionary<string, List<UnitsRadioBehavior>>();

		public UnitsRadioBehavior()
		{
		...
		{
			UnitsRadioBehavior behavior = (UnitsRadioBehavior)bindable;

			if ((bool)newValue)
			{
				string groupName = behavior.GroupName;
				List<UnitsRadioBehavior> behaviors = null;
				...
				foreach (UnitsRadioBehavior otherBehavior in behaviors)
				{
		...
		static void OnGroupNameChanged(BindableObject bindable, object oldValue, object newValue)
		{
			...
			else
			{
				...
				List<UnitsRadioBehavior> behaviors = radioGroups[oldGroupName];
			...
			else
			{
				...
				else
				{
					...
					behaviors = new List<UnitsRadioBehavior>();
		...
		void OnTapRecognizerTapped(object sender, EventArgs args)
		{
			...
			if (label.StyleId.Equals("celsius"))
			{
				app.CalculatorViewModel.DisplayedUnits =
					   VoltageRegulatorTemperature.ViewModels.CalculatorViewModel.Units.Celsius;
			}
			else
			{
				app.CalculatorViewModel.DisplayedUnits =
					   VoltageRegulatorTemperature.ViewModels.CalculatorViewModel.Units.Fahrenheit;
			}
		}
	}
}
